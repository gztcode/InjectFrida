...
class LIEFInject:
    def __init__(self,args):
        ...
        self.deletelist = []
        self.toolPath = os.path.join(getpwd(),"tools")
    ...
    def injectso(self):
        ...
        for soname in injectsolist:
            ...
            so = lief.parse(os.path.join(os.getcwd(),soname))
        ...
        with zipfile.ZipFile(self.apkpath, 'r')as orig_file:
            with zipfile.ZipFile(outapk, 'w')as out_file:
                for item in orig_file.infolist():
                    if item.filename.find(self.soname) != -1 and os.path.exists(os.path.join(os.getcwd(),item.filename+"gadget.so")):
                        out_file.write(os.path.join(os.getcwd(),item.filename+"gadget.so"),arcname=item.filename)
                        if item.filename.find("lib/armeabi-v7a") != -1:
                            out_file.write(os.path.join(self.toolPath,"frida-gadget-16.2.5-android-arm.so"),
                        ...
                        if item.filename.find("lib/arm64-v8a") != -1:
                            out_file.write(os.path.join(self.toolPath, "frida-gadget-16.2.5-android-arm64.so"),
    ...
    def addPort(self,apk_path):
        with zipfile.ZipFile(apk_path, 'a')as apk_file:
            for item in apk_file.infolist():
                if item.filename == "lib/armeabi-v7a/libfrida-gadget.so":
                    apk_file.write(os.path.join(self.toolPath, "libfrida-gadget-port.config.so"),
                                   arcname="lib/armeabi-v7a/libfrida-gadget.config.so")
                    print("add lib/armeabi-v7a/libfrida-gadget-port.config.so")
                if item.filename == "lib/arm64-v8a/libfrida-gadget.so":
                    apk_file.write(os.path.join(self.toolPath, "libfrida-gadget-port.config.so"),
                                   arcname="lib/arm64-v8a/libfrida-gadget.config.so")
                    print("add lib/arm64-v8a/libfrida-gadget-port.config.so")
                if item.filename == "lib/x86/libfrida-gadget.so":
                    apk_file.write(os.path.join(self.toolPath, "libfrida-gadget-port.config.so"),
                                   arcname="lib/x86/libfrida-gadget.config.so")
                    print("add lib/x86/libfrida-gadget-port.config.so")
                continue
    ...
    def signApk(self,apk_path):
        ...
        cmd='jarsigner -verbose -digestalg SHA1 -sigalg SHA1withRSA -storepass %s -keystore %s -signedjar %s %s %s' % (pswd,keystore,outfile,apk_path,alias)
        # cmd = 'java -jar %s sign --ks %s --ks-key-alias %s --ks-pass pass:%s --key-pass pass:%s --out %s %s'% \
            #   (apksignerNew,keystore, alias,pswd,aliaspswd,outfile,apk_path)
        # print(cmd)
        os.system(cmd)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-port', help="HOOK Frida on Port ", action='store_true')
